...
export class PricesFactory {
  static async load(assets) {
    let prices = [];
    try {
      const shouldPriceRefresh = await StorageUtil.getItem("shouldPriceRefresh") || true;
      if (shouldPriceRefresh === false) {
        return await StorageUtil.getItem("noneCoinGeckoPrices");
      }
    ...
    await StorageUtil.setItem("shouldPriceRefresh", false);
    await StorageUtil.setItem("noneCoinGeckoPrices", prices);
    return prices;
  }
}
