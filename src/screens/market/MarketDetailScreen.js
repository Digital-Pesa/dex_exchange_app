...
export default function MarketDetailScreen({ navigation, route }) {
  ...
  const exchange = (value, decimals = 2) => {
    ...
    const number = numeral(value * currency.value).format(format);
    return number;
  };
  return (
          ...
          <View style={styles.content}>
            <View style={styles.coinInfo}>
              <View style={styles.coinInfoUpperArea}>
                ...
                />
                <CommonText style={[styles.coinInfoSymbol, { color: theme.text2 }]}>
                  {coin.symbol.toUpperCase()}
                </CommonText>
              </View>
              <View style={styles.coinInfoLowerArea}>
                <Price style={[styles.coinInfoPrice, { color: theme.text2 }]}>
                  {coin.current_price}
                </Price>
                <View style={styles.coinInfoPercentage}>
                  <View style={[styles.coinInfoPercentageBg, { backgroundColor: theme.background3 }]}>
                    <CommonText
                      style={{
                        color:
            ...
            <View style={styles.coinChart}>
              <YAxis
                ...
                numberOfTicks={5}
                formatLabel={(value) => `${exchange(value)}${currency.symbol}`}
              />
              <View style={{ flex: 1 }}>
                <LineChart
                  style={{ flex: 1 }}
                  data={sparkline}
                  svg={{ stroke: coin.price_change_percentage_24h >= 0 ? "#00ff0a" : "red" }}
                  contentInset={{ top: 20, bottom: 20 }}
                >
                </LineChart>
                <XAxis
            ...
            <View style={styles.coinStatistic}>
              ...
              <View style={[styles.coinStatisticItem, { borderBottomColor: theme.border }]}>
                <CommonText style={{ color: theme.text2 }}>{t("coindetails.rank")}</CommonText>
              ...
              <View style={[styles.coinStatisticItem, { borderBottomColor: theme.border }]}>
                ...
                <Price style={{ color: theme.text2 }}>{coin?.market_cap ?? "-"}</Price>
              </View>
              <View
                style={[[styles.coinStatisticItem, { borderBottomColor: theme.border }], { borderBottomColor: theme.border }]}>
                <CommonText style={{ color: theme.text2 }}>{t("coindetails.volume")}</CommonText>
              ...
              <View style={[styles.coinStatisticItem, { borderBottomColor: theme.border }]}>
                ...
                <Price style={{ color: theme.text2 }}>{coin?.ath ?? "-"}</Price>
              </View>
              <View style={[styles.coinStatisticItem, { borderBottomColor: theme.border }]}>
                <CommonText style={{ color: theme.text2 }}>{t("coindetails.high_24")}</CommonText>
                <Price style={{ color: theme.text2 }}>{coin?.high_24h ?? "-"}</Price>
              </View>
              <View style={[styles.coinStatisticItem, { borderBottomColor: theme.border }]}>
                <CommonText style={{ color: theme.text2 }}>{t("coindetails.low_24h")}</CommonText>
                <Price style={{ color: theme.text2 }}>{coin?.low_24h ?? "-"}</Price>
              </View>
              <View style={[styles.coinStatisticItem, { borderBottomColor: theme.border }]}>
                <CommonText style={{ color: theme.text2 }}>
