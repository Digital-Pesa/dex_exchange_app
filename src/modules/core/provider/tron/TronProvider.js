...
export class TronProvider implements Provider {
  ...
  async estimateGas({ from }) {
    ...
    const { data } = await this.apiInstance.post("wallet/getaccountresource", { address: address });
  ...
  async getTransactions(wallet): Promise<Object> {
    try {
      let transactions = [];
      let temp;
      const { data } = await axios.get(`${configProperties.tron.api2}transaction?sort=-timestamp&count=true&limit=20&start=0&address=${wallet.walletAddress}`);
      if (wallet.type === ASSET_TYPE_TOKEN) {
        temp = _.filter(data.data, function(tx) {
          return tx.contractType === 31 && tx.contractData.contract_address === wallet.contract;
        });
      } else {
        temp = _.filter(data.data, function(tx) {
      ...
      for (let i = 0; i < temp.length; i++) {
        ...
        item.explore = configProperties.tron.explore + `#/transaction/${item.hash}`;
